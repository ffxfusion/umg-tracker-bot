...
module.exports = async (client, guild) => {
    console.log(`[GUILD] ${client.user.tag} has joined ${guild.name} (${guild.id}).`);
    ...
    if (!c) {
        console.log(`[INFO] ${guild.name} (${guild.id}) has no text channels, skipping welcome message. lol`);
    } else {
    ...
    if (!guildProfile) {
        ...
        return console.log(`[DATABASE] ${guild.name} (${guild.id}) has been loaded from the database.`);
    }

    await guildProfile.save().catch(err => console.log(err));
    console.log(`[DATABASE] ${guild.name} (${guild.id}) has been saved to the database.`);
}